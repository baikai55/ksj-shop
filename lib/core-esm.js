...
function imgUrl(cdnBase, rel) {
  ...
  const base = String(cdnBase || "https://cdn.ksjhaoka.com/").replace(/\/?$/, "/");
...
function upstreamMsg(data, fallback = "上游错误") {
...
async function ksjFetch(cfg, pathname, { method = "GET", query, body, form, multipart } = {}) {
  ...
  let payload;
  // 文档：下单等接口 Content-Type = form-data（multipart）
  if (multipart && typeof multipart === "object") {
    const fd = new FormData();
    Object.entries(multipart).forEach(([k, v]) => {
      if (v === undefined || v === null) return;
      fd.append(k, Array.isArray(v) ? JSON.stringify(v) : String(v));
    });
    payload = fd;
    // 不手动设置 Content-Type，由运行时自动带 boundary
...
async function handleApi({ method, pathname, query = {}, body = null, env = {} }) {
  ...
  try {
    ...
    if (m === "GET" && path === "/api/goods" && (query.id || query.goods_id)) {
      requireStore(cfg);
      const id = String(query.id || query.goods_id);
      ...
      if (!result.data.data) return fail(404, "商品不存在或已下架");
    ...
    // 文档：POST https://ym.ksjhaoka.com/api/index/region?goods_id=xxx （Query，不要 body）
    if ((m === "POST" || m === "GET") && path === "/api/region") {
      const goods_id = query.goods_id || body?.goods_id;
      if (!goods_id) return fail(400, "缺少 goods_id（请用 Query：/api/region?goods_id=xxx）");
      ...
      if (result.data.code !== 0) return fail(400, upstreamMsg(result.data, "地区接口失败"), result.data);
    ...
    if (m === "POST" && path === "/api/pool") {
      ...
      if (result.data.code !== 0) return fail(400, upstreamMsg(result.data, "选号失败"), result.data);
    ...
    if (m === "POST" && path === "/api/order") {
      ...
      // 文档 Content-Type: form-data；兼容 urlencoded / json
      let result = await ksjFetch(cfg, "/order", { method: "POST", multipart: payload });
      if (result.httpStatus >= 400 || result.data?.code !== 0) {
        const retryForm = await ksjFetch(cfg, "/order", { method: "POST", form: payload });
        if (retryForm.data?.code === 0) result = retryForm;
        else if (!(result.data && typeof result.data.code === "number")) result = retryForm;
      }
      if (result.httpStatus >= 400 || result.data?.code !== 0) {
        const retryJson = await ksjFetch(cfg, "/order", { method: "POST", body: payload });
        if (retryJson.data?.code === 0) result = retryJson;
      }
      if (result.httpStatus >= 400) return fail(502, "下单请求失败", result.data);
      if (result.data.code !== 0) {
        let msg = upstreamMsg(result.data, "下单失败");
        // 上游“选号失败”常见于渠道自动分配号码失败，不一定是前端漏选号
        if (msg === "选号失败") {
          msg =
            "选号失败（上游渠道自动分配号码失败）。该商品 is_phone=0 时通常无需手动选号；请稍后重试，或换商品/收货地区，并确认姓名与身份证一致。";
        }
